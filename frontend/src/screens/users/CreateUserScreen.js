...
const RegisterScreen = ({history}) => {
  ...
  const userCreated = useSelector((state) => state.userCreate);
  const {success, error:createError,message: createMessage,} = userCreated;
  useEffect(() => {
  ...
  const submitHandler = (e) => {
    e.preventDefault();
    dispatch(
        createUser(name, last_name, email, company_name, phone_number, password)
    );
  };
  ...
  return (<>
      ...
      {success && <Message variant="success">{createMessage}</Message>}
